...
The installer copies config files from `python-setup/config/` into the consumer project. Each config file uses the tool's native `extends`/`inherit` mechanism so the consumer can add project-specific rules without forking the shared config.
...
The shared `.pylintrc` is loaded via `--rcfile`. The installer registers custom checkers in `[tool.pylint.main] load-plugins`. Consumers add project-specific overrides in their own `.pylintrc` or `pyproject.toml`:
