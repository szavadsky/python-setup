...
class TestRecordParsers:
    ...
    def test_mypy_records_sorted_by_file_line(self) -> None:
        stdout = "src/b.py:5: error: X [code-b]\nsrc/a.py:1: error: Y [code-a]\n"
        recs = _parse_mypy_records(stdout)
    ...
    def test_pylint_r0801_collapses_to_one_record(self) -> None:
        ...
        assert (
            "R0801:src/a.py:1-5<->src/b.py:10-15" in rules
        )  # spans sorted canonically
...
class TestCaptureSchemaV2:
    ...
    def test_pylint_captured_as_schema_v2_records(self) -> None:
        cap = _capture_baseline(
            [
                make_lint_result(
                    tool_name="pylint",
                    stdout="src/a.py:1:1: W0611: x (unused-import)\n",
                )
            ]
        )
    ...
    def test_ruff_captured_as_schema_v2_records(self) -> None:
        cap = _capture_baseline(
            [
                make_lint_result(
                    tool_name="ruff check", stdout="src/a.py:1:3: E501 msg\n"
                )
            ]
        )
        assert cap[0]["schema"] == "v2"
    ...
    def test_mypy_captured_as_schema_v2_records(self) -> None:
        cap = _capture_baseline(
            [
                make_lint_result(
                    tool_name="mypy", stdout="src/a.py:1: error: Bad [arg-type]\n"
                )
            ]
        )
        assert cap[0]["schema"] == "v2"
    ...
    def test_unknown_tool_keeps_legacy_output(self) -> None:
        cap = _capture_baseline(
            [make_lint_result(tool_name="strange-tool", stdout="noise\n")]
        )
    ...
    def test_pylint_empty_records_nonempty_stdout_falls_back_to_output(self) -> None:
        ...
        cap = _capture_baseline(
            [
                make_lint_result(
                    tool_name="pylint", stdout="************* Module banner only\n"
                )
            ]
        )
        assert "schema" not in cap[0]
...
class TestDriftResistantDiff:
    ...
    def test_reorder_only_no_spurious_diff(self, tmp_path: Path) -> None:
        ...
        assert {tuple(sorted(r.items())) for r in reloaded[0]["records"]} == {
            ...
            for r in saved[0]["records"]  # type: ignore[attr-defined]
        }
    ...
    def test_exit_code_0_to_nonzero_flagged(self, tmp_path: Path) -> None:
        ...
        current = [
            make_lint_result(
                tool_name="mypy", exit_code=1, stdout="x.py:1: error: m [code]\n"
            )
        ]
        violations, _ = diff_baseline_with(tmp_path, saved, current)
        ...
        saved = [
            {
                ...
                "schema": "v2",
                "records": [
                    {"file": "a.py", "line": 1, "col": None, "rule": "code", "msg": "m"}
                ],
            }
        ]
...
class TestMixedSchemaLoad:
    ...
    def test_legacy_pylint_output_upgraded_to_records_on_diff(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_full_pre_t2_baseline_loads_all_tools(self, tmp_path: Path) -> None:
        ...
        current = [
            make_lint_result(tool_name=t, exit_code=0, stdout="") for t in legacy_tools
        ]
        current.append(
            make_lint_result(
...
class TestPerfBenchmark:
    ...
    def test_50k_line_baseline_compares_under_200ms(self, tmp_path: Path) -> None:
        ...
        assert elapsed_ms < 200.0, (
            f"_compare_sorted took {elapsed_ms:.1f}ms (>200ms ceiling)"
        )
    ...
    def test_diff_baseline_50k_end_to_end_under_1s(self, tmp_path: Path) -> None:
        ...
        for f in range(1000):
            ...
            for line in range(1, 51):
                records.append(
                    {"file": file, "line": line, "col": 1, "rule": "E001", "msg": "m"}
                )
        saved = [
            {"tool": "ruff check", "exit_code": 0, "schema": "v2", "records": records}
        ]
        ...
        assert elapsed_ms < 1000.0, (
            f"_diff_baseline 50k took {elapsed_ms:.1f}ms (>1s ceiling)"
        )
...
class TestDiffBaselineErrors:
    ...
    def test_tool_absent_from_current_removed_from_baseline(
        self, tmp_path: Path
    ) -> None:
        saved = [
...
class TestParserEdgeCases:
    ...
    def test_ty_arrow_form_only(self) -> None:
        ...
        stdout = "error[invalid-argument-type]: Bad arg\n  --> src/a.py:1:3\n"
        recs = _parse_ty_records(stdout)
        assert recs == [Record("src/a.py", 1, 3, "invalid-argument-type", "Bad arg")]
...
class TestFallbackTracking:
    ...
    def test_parser_known_tool_with_traceback_lands_in_fallback(
        self, tmp_path: Path
    ) -> None:
        ...
        captured = _capture_baseline(
            [make_lint_result(tool_name="ruff check", stdout=traceback)]
        )
...
class TestLegacyRstripSet:
    ...
    def test_legacy_addition_count_change_flagged(self, tmp_path: Path) -> None:
        ...
        current = [
            make_lint_result(
                tool_name="strange-tool", stdout="line A\nline A\nline B\n"
            )
        ]
        violations, _ = diff_baseline_with(tmp_path, saved, current)
        assert any("Output changed" in v for v in violations)
...
class TestExitCodeBothNonzero:
    ...
    def test_both_nonzero_diff_exit_codes_still_diff_content(
        self, tmp_path: Path
    ) -> None:
...
class TestMultiSavedToolDedup:
    ...
    def test_duplicate_saved_tool_entries_all_removed_on_tool_absence(
        self, tmp_path: Path
    ) -> None:
...
class TestMixedShapeLegacyOutput:
    ...
    def test_legacy_pylint_with_unparseable_lines_upgrades_partial(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_legacy_pylint_partial_parse_addition_flagged(self, tmp_path: Path) -> None:
        ...
        current_output = (
            saved_output + "src/c.py:5:1: C0114: new (missing-module-docstring)\n"
        )
        saved = [{"tool": "pylint", "exit_code": 0, "output": saved_output}]
...
class TestCaptureBaselineOnDisk:
    ...
    def test_records_emitted_in_sorted_order(self) -> None:
        ...
        cap = _capture_baseline(
            [make_lint_result(tool_name="ruff check", stdout=stdout)]
        )
